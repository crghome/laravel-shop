    ...
    @foreach ($collectProducts as $item)
        <div class="col-3" style="margin-bottom: 30px;">
            <div class="blockItem" style="border: 1px solid #ccc; box-shadow: 0 3px 10px rgba(0,0,0,0.15); height: 100%;">
                ...
                @endphp
                <a href="{{ route('site.shop.product', $item) }}"><IMG src="{{ $img }}" style="display: block; width: 100%; height: 200px; object-position: 50%; object-fit: contain; background-color: {{ \Crghome\Shop\Helpers\HelperLibs::getTrumbColorImage($img, '#fff') }};" /></a>
                <div style="padding: 10px 15px 15px;">
                    <div class="h4" style="margin: 0 0 20px; text-align: center;"><a href="{{ route('site.shop.product', $item) }}">{{ $item->name }}</a></div>
